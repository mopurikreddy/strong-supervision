...
function LM:sample(image_vectors, image_name)
  local N, T = image_vectors:size(1), self.seq_length
  local seq = torch.LongTensor(N, T):zero()
  ...
  torch.save('/data2/mkreddy/BoRD/final_files/codes/densecap/encoded_feats/temp_feats.t7', {feats=image_vecs_encoded})
  --print(image_vecs_encoded:size())
